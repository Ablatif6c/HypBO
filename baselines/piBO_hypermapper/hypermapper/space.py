...
try:
    from hypermapper.utility_functions import (
        concatenate_data_dictionaries, deal_with_relative_and_absolute_path,
        write_data_array)
except ImportError:
    ...
    if ppath:
        ...
        if os.getenv("HYPERMAPPER_HOME"):
            scripts_path.append(os.path.join(
                os.getenv("HYPERMAPPER_HOME"), "scripts"))
...
class RealParameter(Parameter):
    ...
    def __init__(
        ...
        if isinstance(self.prior, list):
            ...
            self.cdf_distribution = self.cdf_distribution / \
                self.cdf_distribution[-1]
...
class IntegerParameter(Parameter):
    ...
    def from_range_0_1_to_parameter_value(self, X):
        ...
        samples = np.round(X_copy * self.get_size() -
                           0.5 + self.get_min()).astype(int)
...
class Space:
    ...
    def get_input_categorical_parameters(self, input_parameters_names_list):
        ...
        """
        return_parameters = []
        input_categorical_parameters = list(
            self.input_categorical_parameters.keys())
    ...
    def get_space(self):
        ...
        for conf in itertool_cartesian_product:  # conf here is a tuple
            ...
            for header in self.get_input_parameters():
                dictionary_of_configurations[header].append(
                    conf[conf_header_index])
                conf_header_index += 1
        ...
        if estimate_prior_flag and use_priors:
            ...
            while len(samples) < size:
                ...
                valid_indices = np.all(
                    [
                        (new_samples[:, i] <
                         input_parameter_objects[k].get_max())
                ...
                if len(samples) == 0:
                    ...
                    samples = np.append(
                        samples, new_samples[(valid_indices)], axis=0)
    ...
    def convert_strings_to_type(self, header, data_list):
        ...
        if param_type == "ordinal":
            ...
            for elem in data_list:
                to_return.append(
                    self.all_input_parameters[header].get_int_value(elem))
            return to_return
    ...
    def random_sample_array(
        ...
        for index in range(len(integer_random_samples)):
            ...
            for var in self.input_output_parameter_names:
                data_array[var].append(
                    all_data_array[var][configuration_index])
    ...
    def random_sample_configurations_without_repetitions(
        ...
        if len(arr) == 0:
            ...
            absolute_configuration_index = np.asarray(
                arr, dtype=np.int64).max() + 1
    ...
    def standard_latin_hypercube_sampling_configurations_without_repetitions(
        ...
        tmp_configurations = []
        # m is the size of the Cartesian product of the categorical variables.
        m = 1
        ...
        lhs_samples = lhs(
            len(self.get_input_non_categorical_parameters(
                input_parameters_names_list)),
            ...
            if param_object.prior == "distribution":
                ...
                X.append(
                    param_object.from_range_0_1_to_parameter_value(
                        np.asarray(stats.beta(a, b).ppf(
                            lhs_samples[:, param_index]))
                    )
                )
        ...
        for i in range(len(X[0])):
            ...
            for j, param_name in enumerate(
                self.get_input_non_categorical_parameters(
                    input_parameters_names_list)
            ):
        ...
        if exploit_prior_information:
            ...
            df_joint_distribution.sort_values(
                by=["joint"], ascending=[0], inplace=True)
    ...
    def get_doe_sample_configurations(
        ...
        if (
            ...
            if doe_type == "random sampling":
                ...
                configurations = self.grid_search(
                    fast_addressing_of_data_array)
    ...
    def run_configurations_client_server(
        ...
        # This is the default case where we want communication via file instead of stdin/out
        read_write_on_a_file = False
            ...
            for conf_index in range(len(configurations)):
                ...
                for parameter in list(input_parameters):
                    ...
                    configuration = {parameter: [
                        configurations[conf_index][parameter]]}
                    str_data += (
                        str((self.convert_types_to_string(
            ...
            if debug:
                ...
                for header in self.get_input_and_output_parameters():
                    if header not in parameters_header:
                        ...
                        print(
                            "The key HyperMapper was looking for is: %s" % str(
                                header)
                        )
                        print("The headers received are: %s" %
                              str(parameters_header))
                        print(
    ...
    def run_configurations_from_data_array(
        ...
        for conf_index in range(len(configurations)):
            ...
            for parameter in list(all_data_array.keys()):
                new_data_array[parameter].append(
                    all_data_array[parameter][index])
        ...
        for output_param in output_parameters:
            if type(objective_values[output_param]) is list:
                ...
                if len(configurations) > 1:
                    ...
                    raise SystemExit
                new_data_array[output_param].append(
                    objective_values[output_param])
    ...
    def filter_already_run_and_fill_with_random_configurations(
        ...
        if len_configurations_to_run < runs_per_optimization_iteration:
            ...
            for configuration in configurations:
                for header in self.get_input_parameters():
                    dictionary_to_be_returned[header].append(
