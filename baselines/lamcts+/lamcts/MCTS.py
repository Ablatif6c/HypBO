...
class MCTS:
    ...
    def __init__(self, lb, ub, dims, ninits, func, Cp=1, leaf_size=20, kernel_type="rbf", gamma_type="auto", init_spaces=None):
        self.dims = dims
        self.samples = []
        self.nodes = []
        self.Cp = Cp
        ...
        self.ROOT = root
        self.CURT = self.ROOT
        self.init_train(init_spaces=init_spaces)
    ...
    def dynamic_treeify(self):
        ...
        while self.is_splitable():
            ...
            for nidx in to_split:
                ...
                assert len(good_kid_data) + \
                    len(bad_kid_data) == len(parent.bag)
    ...
    def init_train(self, init_spaces=[]):

        # here we use latin hyper space to generate init samples in the search space
        init_points = []
        num_init_spaces = len(init_spaces)
        num_init_points_per_space = self.ninits // num_init_spaces

        for init_space in init_spaces:
            lb, ub = zip(*init_space.values())
            lb = np.array(lb)
            ub = np.array(ub)
            init_points_per_space = latin_hypercube(
                num_init_points_per_space, self.dims)
            init_points_per_space = from_unit_cube(
                init_points_per_space, lb, ub)
            init_points.extend(init_points_per_space)

        remaining_points = self.ninits - \
            (num_init_points_per_space * num_init_spaces)
        if remaining_points > 0:
            lb, ub = self.lb, self.ub
            additional_points = latin_hypercube(remaining_points, self.dims)
            additional_points = from_unit_cube(additional_points, lb, ub)
            init_points.extend(additional_points)

        for point in init_points:
            self.collect_samples(point)

        print("="*10 + 'collect ' + str(len(self.samples)) +
              ' points for initializing MCTS'+"="*10)
        print("init_spaces:", init_spaces)
        print("Cp:", self.Cp)
