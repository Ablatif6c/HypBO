...
try:
    from hypermapper import models, space
    ...
    from hypermapper.utility_functions import (
        are_all_elements_equal, compute_data_array_scalarization,
        concatenate_data_dictionaries, create_output_data_file,
        deal_with_relative_and_absolute_path, get_output_data_file,
        get_single_configuration, sample_weight_bbox, sample_weight_flat,
        write_data_array)
...
def main(config, black_box_function=None, profiling=None):
    ...
    if config["resume_optimization"] == True:
        ...
        beginning_of_time = (
            beginning_of_time -
            data_array[param_space.get_timestamp_parameter()[0]][-1]
    ...
    if absolute_configuration_index < number_of_doe_samples:
        ...
        print(
            f"{config['application_name']}: Design of experiment phase, number of new doe samples = {len(configurations)} ......."
        )
    ...
    configurations = []
    evaluation_budget = optimization_iterations * \
        evaluations_per_optimization_iteration
    iteration_number = 0
    evaluation_count = 0
    while evaluation_count < evaluation_budget and run_time / time_budget < 1:
        if evaluation_count % evaluations_per_optimization_iteration == 0:
            iteration_number += 1
            print(f"{config['application_name']}: Starting optimization iteration {iteration_number}")
        ...
        if profiling is not None:
            profiling.add("Model fitting time",
                          (model_t1 - model_t0).total_seconds())
