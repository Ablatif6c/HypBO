...
from .parallel_opt import (disc_acq_KMBBO, disc_acq_max, disc_capitalist_max,
                           disc_constrained_acq_KMBBO,
                           disc_constrained_acq_max)
from .target_space import DiscreteSpace, PartnerSpace, TargetSpace
...
class BayesianOptimization(Observable):
    def __init__(self, pbounds, f=None, random_seed=None, verbose=2,
                 constraints=[]):
        """"""
        ...
        self._space = TargetSpace(f, pbounds, random_seed)
    ...
    def initialize(
        ...
        for _ in range(n_init):
            if self.constraints:
                next_points = [
                    self._space.random_sample(
                        constraints=self.get_constraint_dict()
    ...
    def suggest(self,
        ...
        random_state = self._random_state if seed is None else ensure_rng(seed)
        suggestions = acq_max(
            n=n,
            instance=self,
...
class DiscreteBayesianOptimization(BayesianOptimization):
    '''
    Optimization object by default performs batch optimization \
        of discrete parameters.
    When using the open form optimizer (i.e. writing loops manually)\
        the suggested parameters handled as lists of dicts.
    '''
    ...
    def __init__(self,
        ...
        self._space = DiscreteSpace(f, pbounds, random_seed)
        self.partner_space = PartnerSpace(f, pbounds, random_seed)
    ...
    def initialize(
        ...
        for _ in range(n_init):
            if self.constraints:
                next_points = [x for x in self.constrained_rng(
                    batch,
                    bin=True,)]
            else:
                next_points = [
    ...
    def constrained_rng(
        ...
        if process_seed is not None:
            ...
            seed = int(state[1][0])
            seed += process_seed
            random_state = ensure_rng(random_state=seed)
        ...
        try:
            ...
            raise SyntaxError(
                "Is your liquid constraint lead by the max volume? : {}".format(s))
        ...
        if ms:
            ...
            for i in range(n_points):
                    ...
                    for complement in complements:
                        if 'x[{}]'.format(complement) in s:
                            x[i, complement] = get_rng_complement(
                                n_constrained_var, random_state)
                            ...
                            for reduction in reductions:
                                rem_max_val += pd.eval(reduction,
                                                       local_dict={'x': x[i, :]})
                        else:
                            x[i, complement] = random_state.uniform(
                ...
                rnd = get_rnd_quantities(
                    rem_max_val, n_constrained_var, random_state)
                cnt = 0
                for j in range(n_var):
                    if j in complements:
                        continue
            ...
            while var_list:
                ...
                if 'x[{}]'.format(j) in self.constraints[0]:
                    x[i, j] = np.random.uniform(
                        0, min(bounds[j, 1]-bounds[j, 0], rem_max_val)) + bounds[j, 0]
                    if j in complements:
                        reductions = []
                        p = re.compile(
                            '- \(\(x\[{:d}\]<0.5\) \* \(\(\(0.5 - x\[{:d}\]\)/0.5\) \* \(\d+.\d+-\d+.\d+\) \+ \d+.\d+\) \) '.format(
